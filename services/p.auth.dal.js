...
async function getLoginByUsername(username) {
  let SQL = `SELECT id AS id, username, password, email FROM public."Logins" WHERE username = $1`;
  try {
...
async function getLoginByEmail(email) {
  let SQL = `SELECT id AS id, username, password, email FROM public."Logins" WHERE email = $1`;
  try {
...
async function getLoginById(id) {
  let SQL = `SELECT id AS id, username, password, email FROM public."Logins" WHERE id = $1`;
  try {
